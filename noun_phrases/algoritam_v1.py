...
fig,ax = plt.subplots()
fig.suptitle('Vizualizacija (noun_phrases) prije korekcije sa pravougaonim granicama')
plt.xlabel('X-osa')
plt.ylabel('Frekvencija pojavljivanja rijeci')
r = fig.canvas.get_renderer()
reader = csv.reader(open('ngram_cln.csv'))
...
for key,values in dictionarystart.iteritems():
    ax.text(values[0],values[1],key,size=8.0,
            bbox=dict(facecolor='none',edgecolor='red',pad=1.0))
    
    t = plt.text(values[0],values[1],key,size=8.0,
            bbox=dict(facecolor='none',edgecolor='red',pad=1.0))
...
for key,values in dictionarystart.iteritems():
    ax2.text(values[0],values[1],key,size=8.0)
